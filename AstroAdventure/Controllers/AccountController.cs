...
namespace AstroAdventure.Controllers
{
    public class AccountController : Controller
    {
        ...
        public ActionResult SignUp(tblUser user, string passwordConfirm)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Login(string password, tblUser model)
        {
            // Find user by username only first
            var user = db.tblUsers.FirstOrDefault(u => u.UserName == model.UserName);
                ...
                if (user.PasswordHash == hashed)
                {
                    if (user.IsActive)
                    {
                        ...
                        Session["ProfileImage"] = string.IsNullOrEmpty(user.ProfileImagePath)
                            ? Url.Content("~/Content/Images/default-user.png")
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Setting(tblUser model, HttpPostedFileBase ProfileImage)
        {
            if (ModelState.IsValid)
            {
